...
resource "vsphere_tag_category" "category" {
  name        = "node-discover-test-category"
  cardinality = "SINGLE"
...
resource "vsphere_virtual_machine" "vm" {
  count            = var.vm_count
  name             = "node-discover-test-${element(random_string.vm_name_suffix.*.result, count.index)}"
  ...
  clone {
    ...
    customize {
      linux_options {
        host_name = "node-discover-test-${element(random_string.vm_name_suffix.*.result, count.index)}"
